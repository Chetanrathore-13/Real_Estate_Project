...
export default function PropertyLabelCRUD() {
  ...
  const fetchProperties = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/property/property_labels`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.patch(`${import.meta.env.VITE_API_URL}/property/update_property_labels/${editId}`, formPayload, {
          headers: { 'Content-Type': 'multipart/form-data' , Authorization: token}
        });
      } else {
        await axios.post(`${import.meta.env.VITE_API_URL}/property/add_property_labels`, formPayload, {
          headers: { 'Content-Type': 'multipart/form-data', Authorization: token }
        });
      }
      resetForm();
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this?')) {
      try {
        await axios.delete(`${import.meta.env.VITE_API_URL}/property/delete_property_labels/${id}`, {
          headers: { Authorization: token }
        });
        fetchProperties();
      } catch (error) {
