...
export default function PropertyFeatureCRUD() {
  ...
  const fetchPropertyFeatures = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/property/property_features`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        await axios.patch(`${import.meta.env.VITE_API_URL}/property/update_property_features/${editId}`, formDataToSend, { headers });
      } else {
        await axios.post(`${import.meta.env.VITE_API_URL}/property/add_property_features`, formDataToSend, { headers });
      }
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this feature?")) {
      try {
        await axios.delete(`${import.meta.env.VITE_API_URL}/property/delete_property_features/${id}`, {
          ...
          <TableBody>
            {features.map((feature) => (
              <TableRow key={feature._id}>
                ...
                <TableCell>
                  {feature.icon && (
                    <img src={`data:image/png;base64,${feature.imageBase64
                    }`} alt="Icon" className="h-8 w-8 object-contain" />
                  )}
                </TableCell>
