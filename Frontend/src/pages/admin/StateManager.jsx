...
export default function StateManager() {
  ...
  const addState = async (name, code, icon, description) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/location/add_states`,
        formData,
        {
          headers: {
            Authorization: token,
  ...
  const updateState = async (id, name, code) => {
    try {
      const response = await axios.patch(
        `${import.meta.env.VITE_API_URL}/location/patch_states/${id}`,
        { name, code },
        {
          headers: {
            Authorization: token,
  ...
  const deleteState = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/location/delete_states/${id}`, {
        headers: {
          Authorization: token,
        },
      });
