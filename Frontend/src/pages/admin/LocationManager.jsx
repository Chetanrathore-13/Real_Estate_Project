...
export default function CountryManager() {
  ...
  const addCountry = async ({name, icon, description, code}) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/location/add_country`,
        formData,
        {
          headers: {
            Authorization: token,
  ...
  const updateCountry = async (id, name, code) => {
    ...
    try {
      const respone = await axios.patch(
        `${import.meta.env.VITE_API_URL}/location/patch_country/${id}`,
        {
          name,
          code,
        },
  ...
  const deleteCountry = (id) => {
    try {
      axios.delete(
        `${import.meta.env.VITE_API_URL}/location/delete_country/${id}`,
        {
          headers: {
            Authorization: token,
          },
