...
function BlogForm() {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      if (isEditMode) {
        try {
          const blog = await axios.get(`${import.meta.env.VITE_API_URL}/blog/get_blog/${slug}`, {
    ...
    const fetchCategories = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/blogcategory/blog_categories`,
          {
            headers: { Authorization: token },
          }
        );
  ...
  useEffect(() => {
    const fetchTags = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/blogtag/blog_tags`,
          {
            headers: { Authorization: token },
          }
        );
    ...
    try {
      ...
      if (isEditMode) {
        const response = await axios.patch(
          `${import.meta.env.VITE_API_URL}/blog/update_blog/${slug}`,
          blogData,
          {
            headers: {
              Authorization: token,
        ...
        const response = await axios.post(
          `${import.meta.env.VITE_API_URL}/blog/add_blog`,
          blogData,
          {
            headers: {
              Authorization: token,
