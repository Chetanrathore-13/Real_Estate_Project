...
export default function ProjectTypes() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingSlug) {
      await axios.put(`/api/types/${editingSlug}`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" }
      });
    } else {
      await axios.post("/api/types", formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" }
      });
    }
  ...
  const handleDelete = async (slug) => {
    await axios.delete(`/api/types/${slug}`);
    fetchTypes();
  };
          ...
          types.map((type) => (
            <Card key={type.slug} className="p-4 flex justify-between items-center">
              <CardContent className="flex gap-4 items-center">
                {type.icon && <img src={type.icon} alt={type.title} className="w-12 h-12 rounded-lg object-cover" />}
                <div>
