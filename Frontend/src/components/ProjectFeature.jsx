...
export default function ProjectFeatures() {
  ...
  const fetchFeatures = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/projectFeature/get_projectFeature`,{headers:{Authorization:token}});
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingSlug) {
      await axios.patch(`${import.meta.env.VITE_API_URL}/projectFeature/update_projectFeature/${editingSlug}`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data",Authorization:token }
      });
    } else {
      await axios.post(`${import.meta.env.VITE_API_URL}/projectFeature/add_projectFeature`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data",Authorization:token }
      });
    }
  ...
  const handleDelete = async (slug) => {
    await axios.delete(`${import.meta.env.VITE_API_URL}/projectFeature/delete_projectFeature/${slug}`,{headers:{Authorization:token}});
    fetchFeatures();
  };
