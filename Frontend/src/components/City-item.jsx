...
export function CityItem({ City, onUpdate, onDelete }) {
  const [isEditing, setIsEditing] = useState(false)
  const [name, setName] = useState(City.name)
  const [code, setCode] = useState(City.code)

  const handleUpdate = () => {
    onUpdate(City._id, name, code)
    setIsEditing(false)
  }
    ...
    <li className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden transition duration-300 ease-in-out transform hover:scale-105">
      <div className="p-4">
        {isEditing ? (
          ...
          <div>
            <h3 className="font-semibold text-gray-800 dark:text-white">{City.name}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">{City.code}</p>
          </div>
        )}
      </div>
